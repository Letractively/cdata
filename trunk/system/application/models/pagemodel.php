...
class Pagemodel extends Model{
	...
	function pagemodel(){
    	...
    	"grid" => array(
    		"op" => array(    			
    			"view" => array(
    				"link" => site_url("{$this->object}/xview/page_id")
    			),
    			"id" => $this->object."_id"
    		),
    		"filter" => array(    			
    ...
    function grid_print($pclass = "", $pid = "", $page = 1){
        ...
        $this->myform->list_model["grid"]["op"]["view"]["link"] = "page_xview_page_id.html";
        return $this->myform->generate_grid($page);            
	...
	function view($id){
		...
		$theme = '<h1><?php print $data["title"];?></h1>
<p><?php print $data["teaser"];?></p>
<p><?php print $data["body"];?></p>
