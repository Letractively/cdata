...
class Cdata_user{
  ...
  function check_permission($class = null, $action = null, $id = null){
    ...
    $database = $this->get_database($class, $id);
    
    //only for operator role 4 and dbadmin role 3 and only for db thalassemia
/*
    if($ok == true and ($role == 4 or $role == 3) and ($class == "pform" or $class == "patient") 
    and ($action == "view" or $action == "edit" or $action == "delete" or $action == "add" or $action == "admin")){
...
*/
  ...
  // return patient centre if exist otherwise -1
  function get_database($class = "root", $id = "0"){
        if($class == "root"){
            return -1;
        }
        if($class == "db"){ 
